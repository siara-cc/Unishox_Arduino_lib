...
int unishox2_compress_simple(const char *in, int len, char *out) {
  static const char *freq_seq_dflt[6] = USX_FREQ_SEQ_DFLT;
  static const char *template_dflt[5] = USX_TEMPLATES;
  return unishox2_compress_lines(in, len, UNISHOX_API_OUT_AND_LEN(out, INT_MAX - 1), USX_HCODES_DFLT, USX_HCODE_LENS_DFLT, freq_seq_dflt, template_dflt, NULL);
}
...
int unishox2_decompress_simple(const char *in, int len, char *out) {
  static const char *freq_seq_dflt[6] = USX_FREQ_SEQ_DFLT;
  static const char *template_dflt[5] = USX_TEMPLATES;
  return unishox2_decompress(in, len, UNISHOX_API_OUT_AND_LEN(out, INT_MAX - 1), USX_HCODES_DFLT, USX_HCODE_LENS_DFLT, freq_seq_dflt, template_dflt);
}
